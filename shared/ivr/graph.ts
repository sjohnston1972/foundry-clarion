// Shared between server (server/lib/ivr/graph.ts re-exports this) and client
// (imported directly via the `@shared/*` alias) — the one definition of what an
// IVR flow graph looks like, so the editor and the interpreter can never drift
// on node/edge shape. See tsconfig.app.json / tsconfig.server.json / vite.config.ts.
