// Client mirror of server/lib/ivr/validate.ts — same 5 rules, kept as a deliberate copy
// rather than a shared module (per the design spec's "mirrored in the client" wording),
// since client-side validation runs live on every edit and may reasonably diverge from
// the server's save-time check over time. Only the graph *types* are shared (@shared/ivr/graph);
// if you change a rule here, change it there too (and vice versa).
import { edgesFrom, isTerminalNode, type IvrFlowDefinition, type IvrNode } from '@shared/ivr/graph'
