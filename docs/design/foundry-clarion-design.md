...
| `cc_ivr_flows` | **Native TwiML interpreter flow graphs** (v1, 2026-07-17 — supersedes this row's original "Studio flow reference"; see `docs/superpowers/specs/2026-07-17-ivr-builder-design.md`) | `id` PK, `organization_id`, `name`, `status` (`draft`/`active`), `definition_json`, `updated_at` |
| `cc_voicemails` | Voicemail audio metadata from IVR voicemail nodes (audio in R2, mirrors `cc_recordings`); `flow_id` nullable (`ON DELETE SET NULL`) — best-effort attribution | `id` PK, `organization_id`, `flow_id`, `twilio_call_sid`, `from_e164`, `r2_key`, `duration_s`, `transcript_r2_key`, `transcript_status`, `created_at` |
...
| **IVR builder (v1)** | Visual flow editor (ReactFlow, `src/pages/IvrEditor.tsx`) + Clarion-native TwiML interpreter (`cc_ivr_flows`/`cc_voicemails`, `server/lib/ivr/`, `POST /api/voice/ivr` + `/api/voice/voicemail`) — Studio-free, per the locked decision in CLAUDE.md §7. Flows execute fully under the dry-run rails, exercised by an in-browser simulator; **attaching a flow to a live inbound number is Phase 5 work** (needs `cc_numbers` + live Twilio). | — (completed 2026-07-17; see `docs/superpowers/specs/2026-07-17-ivr-builder-design.md`) |
